...
Proof.
  exists ({[store_loc := LitDict [(LitString "SKU1", row_of 100 10 10)]]} : sn_state),
    [LitString "SKU1"; LitString "ORDER1"; LitInt 5].
  split.
  - unfold gen_pre.
    exists "SKU1", "ORDER1", 5%Z, [(LitString "SKU1", row_of 100 10 10)], 100%Z, 10%Z, 10%Z.
    split; [reflexivity|].
    split; [rewrite lookup_singleton; case_decide; [reflexivity|contradiction]|].
    split; [reflexivity|].
    split; lia.
  - simpl. unfold row_inv.
    exists 100%Z, 10%Z, 10%Z. split; [reflexivity|]. lia.
Qed.
